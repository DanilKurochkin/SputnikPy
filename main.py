...
coat = Coating(As=0.4, epsilon=0.1)
orbit = SunLookingOrbit(radiusAboveEarth=500)

sp = Sputnik(Lx=1,Ly=1, Lz=1, width=0.005, material=mat, coat=coat, orbit=orbit)  
sp.createVolumes(10)

sp.boxes[0].coat = Coating(As=0.3, epsilon=0.9) # ЭКОМ-ж2
sp.boxes[5].coat = Coating(As=0.3, epsilon=0.9)
...
cond2.addEt(wl.TableFunctionPeriodicalHeat([0,0,300,300,0,0], 
                                           [0, orbit.period/4, orbit.period/4 + 1, orbit.period/4 + 1001, orbit.period/4 + 1002, orbit.period],
                                           orbit.period))
